/* A server host that sends files and it's working directory
Author: Aaron Rito
Date: 6/3/2017
Class: CS372 project 2
...
https://cboard.cprogramming.com/linux-programming/78236-list-files-directory-without-folders.html 
 /home/mostafa/netprog/tcpsockex/getfile2_srv.c
*/
#include <stdio.h> 
...
int main(int argc, char* argv[])
{
	 ...
	 while(1) 
	 {
		...
		printf("Data port %d\n", conn_fd);
		 ...
		 if ( (child_pid = fork()) == 0 ) /* fork returns 0 for child EXTRA CREDIT this is multithreaded here. */
		 {
			 close (listen_fd); /* close child's copy of listen_fd */
             /*get the files or directory data*/
			 get_file_name(conn_fd, file_name);
             if(strcmp(file_name, "dir_list") == 0)
             {
                send_listing(conn_fd);
                return 0;
             }
             else
             {
...
int send_file(int sock, char *file_name)
{
     ...
     printf("Done with this client. Sent %zu bytes in %d send(s)\n\n",
...
void get_file_name(int sock, char* file_name)
{
 ...
 void send_listing(int data_sfd)
{
    DIR *dir;
